...
function csvCell(v){if(v===null||v===undefined)return '""';let t=typeof v==='number'?String(v).replace('.',','):String(v);if(typeof v!=='number'&&/^[\s]*[=+@-]/.test(t)&&t!=='-')t="'"+t;return '"'+t.replace(/"/g,'""')+'"';}
function exportWeeklyCSV(){
 const rows=weeklyComparison();if(!rows.length){toast('Não há atualização semanal para exportar.');return;}
 const temporal=temporalExportContext();
 const h=['Contexto temporal','Natureza da visão','Origem temporal','Professor','Turma','Código da turma','Componente','Antes','Agora','Novos lançamentos','Previsto no intervalo','Mínimo 80%','Percentual do previsto','Falta para o mínimo','Situação'];
 const status=r=>r.regressed?'Total reduziu · conferir':r.minimum===null?'Sem carga semanal':r.delta===null?'Sem quantidade comparável':r.met80?'Atingiu ≥ 80%':'Abaixo de 80%';
 const data=rows.map(r=>[temporal.title,temporal.nature,temporal.detail,r.professor,r.shortClass,r.cod_turma,r.componente,r.previous,r.current,r.delta,r.expected,r.minimum,r.ratio===null?null:Math.round(r.ratio*10)/10,r.missingToMinimum,status(r)]);
 const csv='\ufeff'+[h,...data].map(line=>line.map(csvCell).join(';')).join('\r\n');
 downloadFile('DED_'+schoolFileStem()+'_semanal_3tri_2026.csv',csv,'text/csv;charset=utf-8');toast(rows.length+' diários exportados com intervalo e novos lançamentos explícitos.');
}
function exportCSV(){
 if(state.page==='weekly')return exportWeeklyCSV();
...
document.addEventListener('click',e=>{
 ...
 else if(a==='attention-weekly'){state.trimester='3';RAW=getActiveRaw();syncFilterOptions();switchPage('weekly');}
 else if(a==='attention-closure'){state.mode='open';state.teacherGradeMode='all';switchPage('teachers');}
 else if(a==='attention-grades'){state.gradeMode='attention';state.gradePage=0;switchPage('grades');}
 else if(a==='attention-references'){state.mode='unmapped';state.teacherGradeMode='all';switchPage('teachers');}
 else if(a==='attention-integrity'){switchPage('pending');requestAnimationFrame(()=>{const target=document.getElementById('pending-integrity');if(target){target.setAttribute('tabindex','-1');target.scrollIntoView({behavior:'smooth',block:'start'});target.focus({preventScroll:true});}});}
